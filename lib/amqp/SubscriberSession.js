...
function SubscriberSession(options) {
    ...
    this.open = function(channel, consumerTag, next) {
        if (closed) return next(new Error('The session has been closed'))
    ...
    this._hasMessageHandler = function() {
        return self.listeners('message').length > 0;
    }
