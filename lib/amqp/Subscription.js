...
function Subscription(broker, vhost, config, counter) {
    ...
    var unqualifiedSubscriptionName = _.once(deprecations.unqualifiedSubscriptionName)
    ...
    this.subscribe = function(overrides, next) {
        if (config.deprecated) unqualifiedSubscriptionName(config.name, config.vhost)
    ...
    function _subscribe(session, config, next) {
        ...
        vhost.getChannel(function(err, channel) {
            ...
            session._hasMessageHandler() ? attachMessageHandler(channel, session, config)
                                        : attachMessageHandlerLater(channel, session, config)
    ...
    function attachMessageHandlerLater(channel, session, config) {

        var watchForMessageHandler = function(name) {
            if (name !== 'message') return
            session.removeListener('newListener', watchForMessageHandler)
            attachMessageHandler(channel, session, config)
        }

        session.on('newListener', watchForMessageHandler)
    }

    function attachMessageHandler(channel, session, config) {
        if (session.isClosed()) return
        debug('Attaching message handler from %s to channel: %s', config.queue, channel._rascal_id)
        channel.consume(config.source, function(message) {
            ...
            debug('Received message: %s from queue: %s using channel %s', message.properties.messageId, config.queue, channel._rascal_id)
            ...
            if (err) return session.emit('error', err)
            session.open(channel, response.consumerTag, function(err) {
                if (err) return session.emit('error', err)
                timer.reset()
                session.emit('ready');
            })
